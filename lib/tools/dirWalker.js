...
var LESS = path.join(path.sep,'less',path.sep),
	JS = path.join(path.sep,'js',path.sep),
	WIDGET = path.join(path.sep,'widget',path.sep),
	PAGELET = path.join(path.sep,'pagelet',path.sep);
...
function walk(uri,filter,files) {
	...
	if(filter(uri)){
		if(stat.isFile()){
			...
			//打包时排除widget|pagelet|less|js 目录
			if([WIDGET,PAGELET,LESS,JS].every(function(item,i){
