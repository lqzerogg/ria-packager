...
function initJSON(dir, documentRoot, source){
	...
	if(data.testcase.i18n){
		data.testcase.i18n = docbase.getI18N(path.join(documentRoot, source), data.testcase.i18n);	
	}else{
		data.testcase.i18n = docbase.getI18N(path.join(documentRoot, source));	
	}
        	...
        	for(var i=0; i < theme.length; i++){
        		if(theme[i].selected === 'checked'){
        			...
        			data.iframes.push({
		                ...
		                'descript' : description,
		                'url' : iframeUrl + 'main.html?iframe=true&template_data=' + json + url
		            });
        		}	
