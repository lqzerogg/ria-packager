...
var buildTree = function(fileList, filePath,md5Mapping){
		...
		if(typeof str === 'string'){
			while(str){
				...
				if(n === 0){
					...
					ret.children.push(replaceURL(str,uri,filePath,md5Mapping));
					str = '';
				}else{
					ret.children.push(replaceURL(str.substring(0,n),uri,filePath,md5Mapping));
					str = str.substring(n);
				} 
			}
		}else{
			console.log(uri,typeof str)
			if(parentCSS && !missing[parentCSS]){
				missing[parentCSS] = true;
				// console.log(Object.keys(fileList),filePath,uri)
...
var replaceURL = function(str,uri,filePath,md5Mapping){
	return str.replace(urlReg, function(){
		...
		if(hash = md5Mapping[img.split('?')[0]]){
			...
			img = img.substr(0,index) + img.split('?')[0].substr(index).replace('.','_' + hash + '.');; 
...
module.exports = function(filePath, fileList,md5Mapping){
	return clear(buildTree(fileList, filePath,md5Mapping)).join('\n');
};