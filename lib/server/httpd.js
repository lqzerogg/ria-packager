...
app.get("*.less", function(req, res) {
    var file = path.join(config.documentRoot, req.url.split('?')[0]);
    renderLess(file,res,function(css){
        res.header("Content-type", "text/css");
        res.end(css, 'utf-8');
    });
});

function onLessError(e,res,file){
    if(e){
        res.end('<pre style="color:red;">Error when handle ' + file + ' : ' + JSON.stringify(e,null,3) + '</pre>');
        throw e;
    }
}
//render .less file to css text
function renderLess(file,res,callback){
    fs.exists(file, function(exists) {
        if (exists) {
            fs.readFile(file, "utf8", function(err, data) {
                onLessError(err,res,file);
                ...
                var tree, css;
                tree = parser.parse(data,function(e,root){
                    onLessError(e,res,file);
                    try{
                        css = root.toCSS();
                    }catch(error){
                        onLessError(error,res,file);
                    }
        ...
        list.forEach(function(file,index) {
            ...
            case '.css':
                if (fs.existsSync(file)) {
                    ...
                    renderLess(file.replace('.css','.less'),res,function(css){
                        res.write(css, 'utf-8');
                        if(index === length -1){
                            res.end();
                        }
