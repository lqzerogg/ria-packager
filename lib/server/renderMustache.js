...
function showThemeList(project){
    var str = '\n<select name="theme" onchange="_onChange_(this)" >'+
        ...
        '<option value="/project/theme/default/default.css">/theme/default/default.css</option>'+
        '<option value="/project/theme/blue/blue.css">/theme/blue/blue.css</option>'+
    '</select>\n';
...
function showLanguageList(root,project){
    ...
    list.forEach(function(file){
        select.push('<option value="/project/i18n/js/'+ file + '" >' + file +  '</option>');
    });
    select.push('</select>\n');
    var str = select.join('');
    str = str.replace(/project/g,project);
...
//hack handlebars to auto load partial template
function hackHandlebars(baseDir,extension){
    ...
    handlebars.VM.invokePartial = function(partial, name, context, helpers, partials, data){
        if(partial === undefined) {
          partials[name] = partial = fs.readFileSync(path.join(baseDir,name.replace(extension,'').replace(/\./g,path.sep)) + extension,'utf-8').trim();
        }
        return invokePartial.call(handlebars.VM,partial, name, context, helpers, partials, data);
    }
}
...
module.exports = function(req, res,config) {
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
        ...
        handlebars.registerHelper('i18n', function(key) {//I18N in forms of '<div>{{i18n test}}</div>'
        ...
        if (isWidgetOrPagelet) {
            ...
            data.theme_css = req.query.theme || ('/' + project + '/theme/default/default.css');
            data.language_js = req.query.language ||  '/' + project + '/i18n/js/en.js';
            
            handlebars.registerPartial('main_widget_content', content);
