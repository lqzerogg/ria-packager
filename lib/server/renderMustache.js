...
module.exports = function(req, res,config) {
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
        ...
        if (req.url.match(/\/pagelet\/|\/widget\//)) {
            res.write('<link href="SRC" rel="stylesheet">'.replace('SRC', '/'+ project +  templateName.replace('.html', '.css?debug')));
            res.write('<script src="SRC"></script>'.replace('SRC', '/'+ project + '/base/jquery.1.8.1.js'));
            res.write('<script src="SRC"></script>'.replace('SRC', '/'+ project + templateName.replace('.html', '.js?debug')));
        }
