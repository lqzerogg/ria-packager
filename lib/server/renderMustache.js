...
module.exports = function(req, res,config) {
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
        ...
        if(path.basename(templateName) === 'main.html' && !debugWithBackend){
        ...
        if (isWidgetOrPagelet) {
            ...
            if(debugWithBackend){
                ...
                output = output.replace('main_widget_content','{{> '+ templateName.replace('/','')+ ' }}')
                    .replace('global_config','{{> widget/global_config/main.html }}');
            }else{
