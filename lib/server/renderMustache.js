...
module.exports = function(req, res,config) {
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
        ...
        var debugWithBackend = req.query.dev == 'true',
        ...
        if(path.basename(templateName) === 'main.html' && !debugWithBackend && !rawContent && req.query.iframe !== 'true'){
        ...
        if(rawContent){
            output = getMustachePartials(content,root);
        }else{
            if (isWidgetOrPagelet) {
                ...
                if(debugWithBackend){
                    ...
                    .replace('global_config',fs.readFileSync(path.join(root, 'widget/global_config/main.html'
                    .replace(/\//g,path.sep)), 'utf8'));

                    output = getMustachePartials(output,root);
                }else{
