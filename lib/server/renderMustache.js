...
function showThemeList(project){
    var str = '<select name="theme" onchange="_onChange_(this)" >'+
...
function showLanguageList(root,project){
    ...
    var select = ['<select name="language" onchange="_onChange_(this)" > <option value="-1">switch language </option>'];
...
function showJsonDataList(dir){
    if(fs.existsSync(dir)){
        ...
        var select = ['<select name="template_data" onchange="_onChange_(this)" > <option value="-1">switch data for template </option>'];
        list.forEach(function(file){
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
        ...
        if (isWidgetOrPagelet) {
            ...
            var output = Mustache.render(layout, data, function(partial) {//auto load partial template
                return partial === 'main_widget_content' ? content : fs.readFileSync(path.join(root, partial), 'utf8');
            });            
        }else{
