...
module.exports = function(req, res,config) {
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
        ...
        module.require = function(id){
            if(id.match(/\.json$/)){
                return requireJSON(id);
            }else{
                return _require_(id);
            }
        };
        function requireJSON(id){
            id = path.join(root,id);
            var content = fs.readFileSync(id, 'utf-8');
        ...
        data = merge(data,{
            'mustache_page_title' : templateName,
        ...
        var debugWithBackend = req.query.dev == 'true' || req.query.debug == 'true',
        rawContent = req.query.raw == 'true';

        if(!debugWithBackend){
            var globalConfig = path.join(root, 'widget/global_config/_test/main.json'.replace(/\//g,path.sep));
            if(fs.existsSync(globalConfig)){
                data.globalConfig = eval('(' + fs.readFileSync(globalConfig,'utf-8') + ')')['globalConfig'];
            }
        }

        data.main_css = templateName.replace('.html', '.css');
        data.main_js = templateName.replace('.html', '.js');
        data.theme_css = (req.query.theme || ('/less/theme/default.css')).replace("/" + project,'');
        if(req.query.i18n){
            data.language_js = '/js/i18n/' + req.query.i18n + '.js';
            data.language_css = '/less/i18n/' + req.query.i18n + '.css';
        }else{
            data.language_js = '/js/i18n/en.js';
            data.language_css = '/less/i18n/en.css';
        }

        if(req.url.match(/\/page\//)){//page
            data.cdn = "http://" + req.headers.host;
            data.resource = project;
            data.md5_version = new Date().getTime();
        }else{//widget , pagelet ,others
            var prefix = "http://" + req.headers.host + '/'+ project ;
            data.main_css = prefix + data.main_css;
            data.main_js = prefix + data.main_js;
            data.theme_css = prefix + data.theme_css;
            data.language_js = prefix + data.language_js;
            data.language_css = prefix + data.language_css;
        }

        Mustache.clearCache();

        var content = fs.readFileSync(file, 'utf8').trim();
        if(path.basename(templateName) === 'main.html' && !debugWithBackend && !rawContent){
        ...
        var output, isWidgetOrPagelet = req.url.match(/\/pagelet\/|\/widget\//);
        if(rawContent){
            output = content;
        }else{
            if (isWidgetOrPagelet) {
                ...
                if(fs.existsSync(path.join(testDir,'_layout.html'))){
                    layout = fs.readFileSync(path.join(testDir,'_layout.html'), 'utf8');
                ...
                if(debugWithBackend){
                    ...
                    .replace('global_config',fs.readFileSync(
                    path.join(root, 'widget/global_config/main.html'
